...
class FriendService: ObservableObject {
    ...
    func sendFriendRequest(from currentUserId: String, to targetUserId: String) async throws {
        ...
        // Check for existing pending request (current user -> target)
        let outgoingSnapshot = try await db.collection(friendRequestsCollection)
        ...
        // Check for existing pending request (target -> current user)
        let incomingSnapshot = try await db.collection(friendRequestsCollection)
            .whereField("fromUserId", isEqualTo: targetUserId)
            .whereField("toUserId", isEqualTo: currentUserId)
            .whereField("status", isEqualTo: FriendRequestStatus.pending.rawValue)
            .limit(to: 1)
            .getDocuments()

        if !incomingSnapshot.documents.isEmpty {
            throw FriendServiceError.requestAlreadySent
        }
    ...
    func fetchPendingRequests(for userId: String) async {
        do {
            ...
            Logger.friends.error("Failed to fetch pending requests: \(error.localizedDescription)")
        }
    }
    ...
    func fetchSentRequests(for userId: String) async {
        do {
            ...
            Logger.friends.error("Failed to fetch sent requests: \(error.localizedDescription)")
        }
    }
    ...
    func fetchFriends(for userId: String) async {
        do {
            ...
            Logger.friends.error("Failed to fetch friends: \(error.localizedDescription)")
        }
    }
